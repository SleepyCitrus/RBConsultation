...
class RiftboundService:
    ...
    def _get_staples(self) -> list[StapleCard]:
        """
        ...
        return [
            ...
            StapleCard(
                name="Irelia, Fervent", set=SPIRITFORGED, rarity=EPIC, color=GREEN, tcgplayer_id=664885
            ),  # CHECK
            ...
            StapleCard(
                name="Kennen, Storm of Shuriken", set=VENDETTA, rarity=UNCOMMON, color=PURPLE, tcgplayer_id=706051
            ),
    ...
    def get_staple_cards_dict(self) -> dict[str, StapleCard]:
        staple_dict = {}
        for staple in self._get_staples():
            slug = self.get_card_slug(staple.name, staple.set, staple.rarity)
    ...
    #         data = json.load(file)
    #         justtcg_card = JustTCGCard.model_validate(data)
    #         print(justtcg_card)
